        ...
        <div className="container mx-auto px-4 py-3">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              {currentProject?.logo_url ? <img src={currentProject.logo_url} alt={`${currentProject.name} logo`} className="h-10 max-w-[160px] w-auto object-contain" /> : <img src={vecturaLogo} alt="Vectura" className="h-10 w-auto object-contain" />}
    ...
    {/* Main Content */}
      <main className="container mx-auto pt-20 py-[63px] px-0">
        {!currentProject ? <div className="max-w-4xl mx-auto space-y-6">
            {!showProjectsList ? <>
                {/* Mis Proyectos */}
                <Card className="py-0 my-0 mx-0 px-0">
                  ...
                  await fetchUserProjects();
                }} disabled={loadingProjects} className="w-full">
                      ...
                      <form onSubmit={handleProjectAccess} className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <Input id="project-password" type="password" value={projectPassword} onChange={e => setProjectPassword(e.target.value)} placeholder="Introduce la contraseña del proyecto" required />
                        </div>
                ...
                <CardContent>
                  {userProjects.length === 0 ? <div className="text-center py-8">
                      ...
                      {userProjects.map(project => <Card key={project.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => accessProjectDirectly(project)}>
                          <CardHeader className="pb-3">
                            <div className="flex items-center gap-3">
                              {project.logo_url ? <img src={project.logo_url} alt={`${project.name} logo`} className="h-8 w-8 object-contain" /> : <div className="h-8 w-8 bg-primary/20 rounded flex items-center justify-center">
