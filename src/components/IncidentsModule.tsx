...
export default function IncidentsModule({
  ...
  const filtered = useMemo(() => {
    ...
    return incidents.filter(i => (statusFilter ? i.status === statusFilter : true) && (categoryFilter ? i.category === categoryFilter : true) && (term ? [i.name, i.description, i.environment, i.device, i.status, i.category, i.additional_comments].filter(Boolean).some((v: any) => String(v).toLowerCase().includes(term)) : true));
  ...
  const fetchIncidents = async () => {
    setLoading(true);
    try {
      let query = supabase.from('incidents').select('*').eq('project_id', projectId).order('occurred_at', {
        ascending: true
      });
  ...
  const handleUploadEvidence = async (incidentId: string) => {
    ...
    const {
      error
    } = await supabase.storage.from('project-files').upload(filePath, evidenceFile);
    if (error) throw error;
    return filePath;
  };
  ...
  const downloadTemplate = () => {
    const headers = [['Name', 'Description', 'Environment', 'Device', 'OccurredAt(ISO)', 'Status', 'Category', 'Evidence(Url)', 'AdditionalComments']];
  ...
  const loadComments = async (incidentId: string) => {
    const {
      data
    } = await supabase.from('incident_comments').select('*').eq('incident_id', incidentId).order('created_at', {
      ascending: true
    });
    setComments(data || []);
  };
    ...
    <Card>
      ...
      <CardContent>
        ...
        <Table>
          ...
          <TableBody>
            {sorted.map(i => <TableRow key={i.id}>
                ...
                <TableCell>
                  {i.evidence ? i.evidence.startsWith('incidents/') ? <a className="text-primary underline" target="_blank" rel="noreferrer" href="#" onClick={async e => {
                  e.preventDefault();
                  const url = await getUrl(i.evidence);
                  if (url) window.open(url, '_blank');
                }}>
                        Ver archivo
                      </a> : <a className="text-primary underline" target="_blank" rel="noreferrer" href={i.evidence}>Ver enlace</a> : <span className="text-muted-foreground">—</span>}
                </TableCell>
                <TableCell className="flex justify-end gap-2">
                  <Button variant="ghost" size="icon" onClick={async () => {
                  setSelected(i);
                  setDetailsOpen(true);
                  await loadComments(i.id);
                }} aria-label="Ver más">
    ...
    <Dialog open={createOpen} onOpenChange={setCreateOpen}>
      <DialogContent className="max-w-3xl">
        ...
        <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <Label>Entorno</Label>
            <Input value={form.environment} onChange={e => setForm(f => ({
              ...f,
          ...
          <div className="space-y-2">
            ...
            <Input value={form.device} onChange={e => setForm(f => ({
              ...f,
          ...
          <div className="space-y-2">
            ...
            <Input type="datetime-local" value={new Date(form.occurredAt).toISOString().slice(0, 16)} onChange={e => setForm(f => ({
              ...f,
          ...
          <div className="space-y-2">
            ...
            <Input placeholder="https://..." value={form.evidenceLink} onChange={e => setForm(f => ({
              ...f,
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <Textarea value={form.additionalComments} onChange={e => setForm(f => ({
              ...f,
      ...
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <div className="flex items-center justify-between">
            ...
            {selected && <Button variant="ghost" size="icon" aria-label="Editar" onClick={() => {
              onEdit(selected);
              setCreateOpen(true);
        ...
        {selected && <div className="space-y-3">
            ...
            {selected.additional_comments && <div>
            ...
            <div>
              ...
              {selected.evidence ? selected.evidence.startsWith('incidents/') ? <a className="text-primary underline" href="#" onClick={async e => {
              e.preventDefault();
              const url = await getUrl(selected.evidence);
              if (url) window.open(url, '_blank');
            }}>Ver archivo</a> : <a className="text-primary underline" href={selected.evidence} target="_blank" rel="noreferrer">Ver enlace</a> : <span className="text-muted-foreground">—</span>}
            </div>
            <div className="pt-4 border-t">
              ...
              <div className="space-y-3 max-h-48 overflow-auto mt-2 pr-1">
                {comments.map(c => <div key={c.id} className="rounded-md border p-2">
