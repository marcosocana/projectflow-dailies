-- Rebuild today's daily view from Home assignments even when the daily row
-- did not exist before the previous restore migrations ran.

WITH projects_with_assigned_work AS (
...
SELECT
  ...
  '{}'::jsonb
FROM projects_with_assigned_work
WHERE NOT EXISTS (
  SELECT 1
  FROM public.dailies d
  WHERE d.project_id = projects_with_assigned_work.project_id
    AND d.date = timezone('Europe/Madrid', now())::date
);
...
inserted_tasks AS (
  ...
  WHERE NOT EXISTS (
    SELECT 1
    FROM existing_tasks
    WHERE existing_tasks.daily_id = desired_sources.daily_id
  )
  RETURNING id, daily_id
),
all_desired_tasks AS (
  SELECT daily_id, task_id FROM existing_tasks
  UNION
  SELECT daily_id, id AS task_id FROM inserted_tasks
),
missing_links AS (
  SELECT
    all_desired_tasks.daily_id,
    all_desired_tasks.task_id,
    ROW_NUMBER() OVER (
      PARTITION BY all_desired_tasks.daily_id
      ORDER BY all_desired_tasks.task_id
    ) AS restore_order
  FROM all_desired_tasks
  WHERE NOT EXISTS (
    SELECT 1
    FROM public.daily_tasks existing
    WHERE existing.daily_id = all_desired_tasks.daily_id
      AND existing.task_id = all_desired_tasks.task_id
  )
),
max_orders AS (
  SELECT
