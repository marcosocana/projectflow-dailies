-- Restore task links that were omitted from the 2026-05-26 daily persistence.
-- This only recreates missing daily_tasks rows from 2026-05-25 and never
-- deletes or changes task content.
...
missing_tasks AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY COALESCE(source_tasks.order_position, 999999), tasks.created_at
    ) AS restore_order
  FROM daily_pairs
