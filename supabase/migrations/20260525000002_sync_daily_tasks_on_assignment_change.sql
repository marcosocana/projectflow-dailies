...
BEGIN
  ...
  FOR v_assignment IN
    SELECT DISTINCT ON (assigned_to) assigned_to, status
  ...
  LOOP
    v_task_status := CASE
      WHEN v_assignment.status IN ('resolved'::public.incident_status, 'closed'::public.incident_status)
        THEN 'resolved'::public.task_status
      WHEN v_assignment.status IN ('in_progress'::public.incident_status, 'in_qa'::public.incident_status)
        THEN 'in_progress'::public.task_status
      ELSE 'pending'::public.task_status
    END;
    v_task_environment := CASE
      WHEN v_incident.environment IN ('DEV', 'PRE', 'PRO') THEN v_incident.environment
      ELSE NULL
    END;
...
CREATE OR REPLACE FUNCTION public.sync_daily_tasks_after_assignment_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  PERFORM public.sync_daily_tasks_for_incident_assignments(COALESCE(NEW.incident_id, OLD.incident_id));
  RETURN COALESCE(NEW, OLD);
END;
$$;

DROP TRIGGER IF EXISTS sync_daily_tasks_after_assignment_change ON public.incident_assignments;
CREATE TRIGGER sync_daily_tasks_after_assignment_change
AFTER INSERT OR UPDATE OR DELETE ON public.incident_assignments
FOR EACH ROW
EXECUTE FUNCTION public.sync_daily_tasks_after_assignment_change();
