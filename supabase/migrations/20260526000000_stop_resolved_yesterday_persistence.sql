-- Stop carrying resolved daily tasks forward with a separate historical status.
-- Resolved tasks stay resolved and are excluded from daily persistence.

UPDATE public.tasks
SET
  status = 'resolved'::public.task_status,
  status_environment = COALESCE(status_environment, 'PRO')
WHERE status = 'resolved_yesterday'::public.task_status;
  ...
  LOOP
    ...
    FOR v_task_record IN
      ...
      WHERE dt.daily_id = v_source_daily_id
        AND t.status NOT IN ('resolved'::public.task_status, 'resolved_yesterday'::public.task_status)
