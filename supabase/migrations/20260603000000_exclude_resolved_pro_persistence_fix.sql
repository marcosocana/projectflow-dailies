-- Resolved tasks already in PRO must not be carried into the next daily.
-- The cleanup removes only links copied from the source daily into today's daily.
...
BEGIN
  ...
  WHERE target_dt.daily_id = target_daily.id
    ...
    AND t.status = 'resolved'::public.task_status
